...
describe('AuthService.register', () => {
  ...
  it('hashes the password before creating a user and returns a token', async () => {
    ...
    (usersRepository.createUser as jest.Mock).mockImplementation(async (dto: any) => ({
      id: 1,
      role: Role.CUSTOMER,
      ...dto,
    }));
    ...
    const createdUserPayload = (usersRepository.createUser as jest.Mock).mock.calls[0][0];
    expect(createdUserPayload.password).not.toBe('Password123');
    expect(await bcrypt.compare('Password123', createdUserPayload.password)).toBe(true);
